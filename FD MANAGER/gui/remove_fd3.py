...
def remove_fd(entry_id, treeview):
    id = entry_id.get()
    ...
    if record:
        ...
        if confirm:
            delete_record(id)
            load_data(treeview)  # Refresh the data
    else:
        messagebox.showerror("Error", "Record not found. Please make sure the ID exists.")

# Function to open the remove FD window
def open_remove_fd_window(parent_window):
    # Create a new top-level window
    window = tk.Toplevel(parent_window)
    window.title("Remove Fixed Deposit")
    window.geometry("600x400")  # Adjust size as needed

    # Create the Treeview widget to display the records
    ...
    button_remove = tk.Button(window, text="Remove FD", command=lambda: remove_fd(entry_id, treeview))
    button_remove.pack()

    # Load data to show existing records
    load_data(treeview)

    # Run the Tkinter event loop for this window
    window.mainloop()
