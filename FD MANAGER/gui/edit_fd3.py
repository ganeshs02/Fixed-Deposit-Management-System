...
def load_fd_details(entry_fd_id, entry_bank_name, entry_deposit_amount, entry_start_date, entry_maturity_date, entry_interest_rate):
...
def save_fd_details(entry_fd_id, entry_bank_name, entry_deposit_amount, entry_start_date, entry_maturity_date, entry_interest_rate):
    fd_id = entry_fd_id.get()
...
def run_edit_fd():
    ...
    # Buttons for actions
    tk.Button(window, text="Load Details", command=lambda: load_fd_details(entry_fd_id, entry_bank_name, entry_deposit_amount, entry_start_date, entry_maturity_date, entry_interest_rate)).grid(row=6, column=0, columnspan=2)
    tk.Button(window, text="Save Changes", command=lambda: save_fd_details(entry_fd_id, entry_bank_name, entry_deposit_amount, entry_start_date, entry_maturity_date, entry_interest_rate)).grid(row=7, column=0, columnspan=2)

    # Run the Tkinter event loop
    window.mainloop()

# Function to open the window externally
def open_edit_fd_window():
    run_edit_fd()

# Only run the Tkinter window if this file is executed directly
